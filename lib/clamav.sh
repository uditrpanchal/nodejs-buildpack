...
configure(){
    ...
    local build_dir=${1:-}
    $build_dir/clamav-0.99.2/configure --with-user=vcap --prefix=/home/vcap/app/clamav --disable-clamav --with-system-llvm=$build_dir/clang+llvm-3.6.0-x86_64-linux-gnu/bin/llvm-config
...
compile(){
      echo "compile clamav"
     local build_dir=${1:-}
     make -C $build_dir/clamav_0.99.2
}

install(){
    echo "install clamav"
    local build_dir=${1:-}
    make  -C $build_dir/clamav_0.99.2 install
}
